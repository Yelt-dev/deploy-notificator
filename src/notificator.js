...
const { mailList } = require('./config/mail.list.config')

const app = express();

const server = app.listen(0, () =>{
    console.log(`Email sender listening on port ${server.address().port}`);
    const emailTemplate = fs.readFileSync('./src/templates/emailTemplate.ejs', 'utf8');
    ...
    const renderedTemplate = ejs.render(emailTemplate, {
        ...
        body: `Se realizó un nuevo deploy para la aplicación ${dataConfig.app_deploy_name} con fecha y hora: ${new Date().toLocaleDateString()}  ${new Date().toLocaleTimeString()}`,
